...
public class FastDFSUploader extends Uploader {
    ...
    public void doUploadFileChunk(QiwenMultipartFile qiwenMultipartFile, UploadFile uploadFile) throws IOException {
        ...
        if (uploadFile.getChunkNumber() <= 1) {
            ...
            // 记录第一个分片上传的大小
            redisUtil.set(uploadFile.getIdentifier() + "_uploaded_size", qiwenMultipartFile.getSize(), 1000 * 60 * 60);
            ...
            // 记录分片上传的大小
            redisUtil.set(uploadFile.getIdentifier() + "_uploaded_size", alreadySize + qiwenMultipartFile.getSize(), 1000 * 60 * 60);
